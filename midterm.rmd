...
<span style="color: green;">The data contains trends that are not well-reflected in the chains.</span>
...
```{r}
#Check the posterior distribution of the informative prior
#adjust the prior slope to 0.7 and sd = 0.01
new_prior <- normal(location = c(0.7, 0.01), scale = c(2.5, 1))


new_model <- stan_glm(Culmen..mm. ~ Tarsus..mm., data= quail_data, prior = normal(location = 0.7, scale = 0.01), chains=4)
                  
new_posterior <- as.matrix(new_model)

mcmc_areas(new_posterior,
           pars = c("Tarsus..mm."))

```

```{r}
###Check the posterior distribution of the default(uninformed) prior

model <- stan_glm(Culmen..mm. ~ Tarsus..mm., data= quail_data, chains=4)
...
```{r}
library(modelr)
  BRMquail = brm(Culmen..mm. ~ Tarsus..mm., data = quail_data)


  
samps10 <-  quail_data %>%
    data_grid(Tarsus..mm. = seq_range(Tarsus..mm., n = 10))  %>%
    add_fitted_draws(quail_brm) %>%
  sample_draws(1) %>% as_tibble()

prior_ten <- fixef(samps10)

samps100 <- quail_data %>%
    data_grid(Tarsus..mm. = seq_range(Tarsus..mm., n = 100))  %>%
    add_fitted_draws(quail_brm) %>%
  sample_draws(1)

samps300 <- quail_data %>%
    data_grid(Tarsus..mm. = seq_range(Tarsus..mm., n = 300))  %>%
    add_fitted_draws(quail_brm) %>%
  sample_draws(1)

samps500 <- quail_data %>%
    data_grid(Tarsus..mm. = seq_range(Tarsus..mm., n = 500))  %>%
    add_fitted_draws(quail_brm) %>%
  sample_draws(1)

prior_summary(quail_brm)

ggplot() +
    geom_density(data=samps10, mapping=aes(x=.value), color="black", alpha = 0.7) +
  geom_density(data=samps100, mapping=aes(x=.value), color="red", alpha = 0.7) +
  geom_density(data=samps300, mapping=aes(x=.value), color="green", alpha = 0.7) +
  geom_density(data=samps500, mapping=aes(x=.value), color="orange", alpha = 0.7) +
  theme_bw()
...
#####THISSSS isss going to be the one

fnc_smple <- function(x)

sample <-  quail_data[sample(nrow(quail_data), x), ]

model <- stan_glm(Culmen..mm. ~ Tarsus..mm., data= sample, chains=1)
