...
<span style="color: green;">I am inclined towards inferential thought processes that resemble Bayesian probabilistic thinking, as I tend to make causal inferences from observing patterns. Further, this inferential thought process is influenced by prior knowledge which is analguous to a conditional probability. 
...
<span style="color: green;">The residuals vs. fitted plot upholds the assumption that residuals and fitted values are independent. The is evident by the cloud formation around the 0 line.</span>

```{r}
plot(LSquail, which =2)
```

<span style="color: green;">This qqplot maintains the assumption of normally distributed data, as the residuals 
...
set.seed(600)
tarsus_brm <- brm(Culmen..mm. ~ Tarsus..mm.,
                  data = quail_data,
                  family = gaussian(link = "identity"))
...
tarsus_posterior <- posterior_samples(tarsus_brm, add_chain = T)

head(tarsus_posterior)

```

```{r}
mcmc_trace(tarsus_posterior)+
  scale_color_manual(values = c("red", "blue", "orange", "yellow"))
...
mcmc_acf(tarsus_posterior)

```

```{r}
tarsus_fit <- fitted(tarsus_brm) %>% as.data.frame()
tarsus_res <- residuals(tarsus_brm) %>% as.data.frame()

plot(tarsus_fit$Estimate, tarsus_res$Estimate)
...
qqnorm(tarsus_res$Estimate)
qqline(tarsus_res$Estimate)
...
```{r}
tarsus_brm_prior <- brm(Culmen..mm. ~ Tarsus..mm.,
                         data = quail_data,
                         family=gaussian(),
                   prior = c(prior(normal(0.7, 0.01), class = Intercept),
                             prior(normal(1, 1), class = b),
                             prior(uniform(3, 3.4), class = sigma)))
